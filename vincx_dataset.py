...
class VinDataset(Dataset):
    def __init__(self, basedir, fold, is_training, img_size=512,\
        ...
        self.df = pd.read_csv(self.basedir + 'train_folds.csv')
        self.categories = ['Normal', 'Not_normal'] # for classification on the whole image
        # self.samples = self.df[self.df.fold != fold]
        # self.samples = self.df[self.df.fold != fold]
        # self.samples = self.samples.head(int(len(self.samples) * 0.01)) # debug        
        
        if is_training:
            self.samples = self.df[self.df.fold != fold]
            self.samples = self.samples.head(int(len(self.samples) * 0.01)) # debug
        else:
            self.samples = self.df[self.df.fold == fold] # debug
            self.samples = self.samples.sample(int(len(self.samples) * 0.001)) # debug

            # self.samples = self.df[self.df.fold == fold]

        self.patient_ids = list(sorted(self.samples.image_id.unique()))
        # self.categories = list(sorted(self.samples.))

        # self.class2id = {k: v for k, v in self.samples.iterrows()}
        self.patient_categories = {} # for classify the whole image
        self.annotations = defaultdict(list)
        self.classids = defaultdict(list)
        # self.annotations_orig = defaultdict(list)

        for _, row in self.samples.iterrows():
            patid = row['image_id']
            self.patient_categories[patid] = self.categories.index(row['class'])
            # self.patient_categories[patid] = row['class_id']

            if row.class_id != 14:
                xmin, ymin, xmax, ymax = row.x_min / row.width, \
                                            row.y_min / row.height, \
                                            row.x_max / row.width, \
                                            row.y_max / row.height

                x = xmin * self.img_size
                y = ymin * self.img_size
                w = xmax * self.img_size - xmin * self.img_size
                h = ymax * self.img_size - ymin * self.img_size
                ...
                # print('points: ', points)
                self.classids[patid].append(row['class_id'])
                self.annotations[patid].append(points)
                # annotorig = [xmin * self.img_size, \
                #             ymin * self.img_size, \
                #             xmax * self.img_size, 
                #             ymax * self.img_size
                #             ]
                # self.annotations_orig[patid].append(annotorig)
    ...
    def __getitem__(self, idx):
        # patid = self.patient_ids[idx]
        patid = self.samples.image_id.values[idx]

        imgpath = self.basedir + 'train/' + patid + '.png'
        img = Image.open(imgpath)
        img_source_w, img_source_h = self.img_size, self.img_size
        ...
        for annotation, classid in zip(self.annotations[patid], self.classids[patid]):
        ...
        if len(annotations):
            ...
            annotations = np.zeros((0, 5)) #array([], shape=(0, 5), dtype=float64)
        # annotorig = np.row_stack(self.annotations_orig[patid]) 

        sample = {'img': crop,
                    'annot': annotations,
                    'scale': 1.0, 
                    # 'category': self.classids[patid]
                    'category': self.patient_categories[patid]
                    # 'annotorig': annotorig
                }

        return sample 

def check_vindataset():
    with utils.timeit_context('load ds'):
        basedir = '/media/basic/ssd256/VinBigData_CXR/vinbigdata/'
        ds = VinDataset(basedir, fold=0, is_training=True, img_size=512)

    # annotorigs = ds[3]['annotorig']
    # plt.show()
    print('category: ', ds[3]['category'])
...
def check_vinperformance():
    ...
    dataloader_train = torch.utils.data.DataLoader(ds, num_workers=8, batch_size=8,
                                  shuffle=True,
...
if __name__ == '__main__':
    ...
    # check_performance()
    check_vindataset()
    # check_vinaugmentations()
    # check_vinperformance()