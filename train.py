...
from vincx_dataset import VinDataset
...
MODELS = {
    ...
    'se_resnext101_512': ModelInfo(
        ...
        args=dict(num_classes=15, pretrained='imagenet'),
        img_size=512,
        batch_size=3,
        dataset_args=dict()
    ),
    'se_resnext101_dr_512': ModelInfo(
        factory=pytorch_retinanet.model_se_resnext.se_resnext101,
        args=dict(num_classes=15, pretrained='imagenet', dropout=0.5),
        img_size=512,
        batch_size=4,
        dataset_args=dict(augmentation_level=20)
    ),
    'se_resnext101_dr0.75_512': ModelInfo(
        factory=pytorch_retinanet.model_se_resnext.se_resnext101,
        args=dict(num_classes=15, pretrained='imagenet', dropout=0.75),
        img_size=512,
        batch_size=4,
        dataset_args=dict(augmentation_level=20)
    ),
    'se_resnext101_dr_512_without_pretrained': ModelInfo(
        factory=pytorch_retinanet.model_se_resnext.se_resnext101,
        args=dict(num_classes=15, pretrained=False, dropout=0.5),
        img_size=512,
        batch_size=4,
        dataset_args=dict(augmentation_level=20)
    ),
    ...
    'se_resnext50_512_dr0.8': ModelInfo(
        factory=pytorch_retinanet.model_se_resnext.se_resnext50,
        args=dict(num_classes=15, pretrained='imagenet', dropout=0.8),
        img_size=512,
        batch_size=8,
        dataset_args=dict(augmentation_level=20)
    ),
...
def train(model_name, fold, run=None, resume_weights='', resume_epoch=0, \
    ...
    dataset_train = VinDataset(basedir=basedir, fold=fold, img_size=model_info.img_size, is_training=True, **model_info.dataset_args)
    dataset_valid = VinDataset(basedir=basedir, fold=fold, img_size=model_info.img_size, is_training=False)
    ...
    # else:
    optimizer = optim.Adam(retinanet.parameters(), lr=3e-4)
...
def check(model_name, fold, checkpoint, \
    ...
    dataset_valid = VinDataset(basedir=basedir, fold=fold, \
                                img_size=model_info.img_size, is_training=False)
    ...
    for iter_num, data in data_iter:
        ...
        # print('nms_scores: ', nms_scores.shape) # []
        transformed_anchors = transformed_anchors.cpu().detach().numpy()

        print('nms_scores, transformed_anchors: ', nms_scores.shape, transformed_anchors.shape)
        ...
        print('---GT---')
        print('annots: ', data['annot'].shape)

        print('---PR---')
        print('transformed_anchors: ', transformed_anchors.shape)
        print('nms_class: ', nms_class.shape, nms_class)
        # print('bbox classes?')
        ...
        for gt in data['annot'].cpu().detach().numpy():
            for i in range(gt.shape[0]):
                if np.all(np.isfinite(gt[i])):
                    p0 = gt[i, 0:2]
                    p1 = gt[i, 2:4]
                    plt.gca().add_patch(
                        plt.Rectangle(p0, width=(p1 - p0)[0], height=(p1 - p0)[1], 
        ...
        for i in range(len(nms_scores)):
            ...
            plt.gca().text(p0[0], p0[1], f'{nms_class[i]}: {nms_score:.3f}', color=color)  # , bbox={'facecolor': color, 'alpha': 0.5})
        plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='se_resnext101_512')
    parser.add_argument('--run', type=str, default='')
    parser.add_argument('--fold', type=int, default=0)
    parser.add_argument('--weights', type=str, default='')
    parser.add_argument('--epoch', type=int, default=0)
    parser.add_argument('--from-epoch', type=int, default=0)
    parser.add_argument('--to-epoch', type=int, default=8)
    ...
    if action == 'check':
        if args.epoch > -1:
            ...
            weights = f'checkpoints/{model}{run_str}_fold_{fold}/{model}_{args.epoch:03}.pt'
        else:
            weights = args.weighs
