...
def nms_vin(dets, scores, thresh):
    "Dispatch to either CPU or GPU NMS implementations.\
    Accept dets as tensor"""
    # return pth_nms(dets, thresh)
    return nms(dets, scores, thresh)
...
class ClassificationModel(nn.Module):
    def __init__(self, num_features_in, num_anchors=9, num_classes=15, prior=0.01, feature_size=256, dropout=0.5):
...
class GlobalClassificationModel(nn.Module):
    def __init__(self, num_features_in, num_classes=2, feature_size=256, dropout=0.5):
        super().__init__()
...
class RetinaNet(nn.Module):
    def __init__(self, encoder: RetinaNetEncoder,
        ...
        self.classificationModel = ClassificationModel(256, num_classes=15, dropout=dropout_cls)
        
        #added another classification output 
        #predicting the class 
        #of the whole image ('No Lung Opacity / Not Normal', 'Normal', 'Lung Opacity')
        ...
        self.globalClassificationModel = GlobalClassificationModel(fpn_sizes[-1], num_classes=2, feature_size=256, dropout=dropout_global_cls)
    ...
    # def boxes(self, img_batch,
    #                 regression,
    #                 classification,
    #                 global_classification, anchors, is_training=True):
    ...
    #     # print('\nclassification: ', classification.shape[2])
    #     # do NMS on every class
    #     for i in range(classification.shape[2]):
    #         scores = torch.squeeze(classification[:, :, i])
    #         # print('\nscores: ', scores.shape, scores.max())
    #         scores_over_thresh = (scores > 0.015) # 0.015
    #         # print('scores_over_thresh: ', scores_over_thresh.shape, scores_over_thresh.sum())
    #         if scores_over_thresh.sum() == 0:
    #             # no boxes to NMS, just return
    #             print('\nno bboxes! on class:', i)
    #             continue
    #             # if is_training:
    ...
    #         # print('anchors_nms_idx: ', anchors_nms_idx.shape)
    #         print('anchorBoxes: ', anchorBoxes.shape, anchorBoxes[anchors_nms_idx].shape)
    ...
    #         print('finalAnchorBoxesCoordinates: ', finalAnchorBoxesCoordinates.shape)
        
    #     return [finalScores, global_classification, finalAnchorBoxesCoordinates]
    ...
    def boxes(self, img_batch, regression, classification, global_classification, anchors):
        ...
        print('threshold scores_over_thresh: ', 0.055)
        scores_over_thresh = (scores > 0.055)[0, :, 0] # 0.025
        # scores_over_thresh = (scores > 0.005)[0, :, 0]

        if scores_over_thresh.sum() == 0:
            # no boxes to NMS, just return
            return [torch.zeros(0), global_classification, torch.zeros(0, 4)] # train()
        #     continue # eval()
        else:
            ...
            # print('scores: ', scores.shape, scores[0, :, 0].shape)
            # print('transformed_anchors: ', transformed_anchors.shape, transformed_anchors[0, :, :].shape)
            # scores = scores.squeeze()
            anchors_nms_idx = nms_vin(transformed_anchors[0, :, :], scores[0, :, 0], 0.05)
    ...
    def forward(self, inputs, return_loss, return_boxes, return_raw=False):
        ...
        # print('global_classification: ', global_classification.shape)
        # print('anchors: ', anchors.shape)
        # classification:  torch.Size([1, 196560, 15])
        # global_classification:  torch.Size([1, 2])
        # anchors:  torch.Size([1, 196560, 4])
